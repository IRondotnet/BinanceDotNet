...
namespace BinanceExchange.API
{
   ...
   public class APIProcessor : IAPIProcessor
   {
      ...
      private DateTime? _timestampOffsetSetDateTime;
      ...
      /// <returns></returns>
      public async Task<T> ProcessGetRequest<T>(BinanceEndpointData endpoint, int receiveWindow = 10000) where T : class
      {
      ...
      /// <param name="receiveWindow"></param>
      /// <returns></returns>
      public async Task<T> ProcessDeleteRequest<T>(BinanceEndpointData endpoint, int receiveWindow = 10000) where T : class
      {
         var fullKey = $"{typeof(T).Name}-{endpoint.Uri.AbsoluteUri}";
         if (_cacheEnabled && endpoint.UseCache)
         {
      ...
      private async Task<TimeSpan> GetValidTimestampOffsetAsync()
      {
         if (!_timestampOffsetSetDateTime.HasValue || DateTime.UtcNow - _timestampOffsetSetDateTime >= TimeSpan.FromMinutes(5))
         {
            var serverTimeResponse = await ProcessGetServerTimeRequest(Endpoints.General.ServerTime);
            _timestampOffset = serverTimeResponse.ServerTime - DateTime.UtcNow;
            _timestampOffsetSetDateTime = DateTime.UtcNow;
         }
